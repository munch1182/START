        ...
        <Table class="w-full border-collapse" :class="{
            ...
            <TableHeader>
                <TableRow class="bg-gray-50">
                    <TableHead v-for="title in normalizedTitles" :key="title"
                        class="px-4 py-3 font-semibold text-gray-700 text-left border-b border-gray-200" :class="{
                            'px-3 py-2': compact
                        }">
                        {{ title }}
                    </TableHead>
                    <TableHead v-if="showActionColumn"
                        class="px-4 py-3 font-semibold text-gray-700 text-center border-b border-gray-200 w-24" :class="{
                            'px-3 py-2': compact
                        }">
            ...
            <TableBody>
                <TableRow v-if="!hasData">
                    <TableCell :colspan="normalizedTitles.length + (showActionColumn ? 1 : 0)"
                        class="px-4 py-8 text-center text-gray-500 italic border-b border-gray-200" :class="{
                            'px-3 py-6': compact
                        }">
                        {{ emptyMessage }}
                    </TableCell>
                ...
                <TableRow v-for="(item, index) in normalizedData" :key="item[keyField] || item.id || index"
                    ...
                    <TableCell v-for="title in normalizedTitles" :key="title"
                        class="px-4 py-3 text-gray-900 border-b border-gray-200" :class="{
                            'px-3 py-2': compact,
                            'truncate': compact
                        }">
                        <!-- 支持自定义单元格内容 -->
                    ...
                    <TableCell v-if="showActionColumn" class="px-4 py-3 text-center border-b border-gray-200" :class="{
                        'px-3 py-2': compact
